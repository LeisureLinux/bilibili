...
old_key=$(awk '{print substr($(NF - 1 ), length($(NF -1 )) - 8 )}' "$PUB")
...
# echo "Old Key ID: $old_key_id, Old Key: $old_key"
...
# Remove Old Key
ssh -o PasswordAuthentication=no -i $PRIV.old $host sed -i "/$old_key/d" $AUTH_KEY
# Verify New Key
